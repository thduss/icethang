...
while cap.isOpened():
    ...
    if face_result.multi_face_landmarks:
        ...
        if not baseline_done:
            ...
            if eyes_too_long_closed:
                focused = False; final_status = "SLEEPING"; status_color = (0, 0, 255); gaze_fail_count = 0
            ...
            # 4. 움직임 감지 -> 집중 아님(False)
                ...
                if gaze_focused:
                    final_status = "MOVING (Focus Kept)" # 시선은 있는데 움직임 -> 실패
                else:
                    final_status = "MOVING (Gaze Off)"   # 시선도 없고 움직임 -> 실패
            ...
            # 움직임 없고 시선 나감 -> 집중 실패
                ...
                if gaze_fail_count > GAZE_FAIL_LIMIT:
                    focused = False; final_status = "GAZE OFF"; status_color = (0, 0, 255)
                else:
    ...
    cv2.imshow("Focus Detection - Strict Movement", image)
