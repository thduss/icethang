...
interface StudentInfo { id: number; name: string; classId: number; }
...
export default function NormalClassScreen() {
  ...
  const studentInfo: StudentInfo = { id: studentData?.studentId || 4, name: studentData?.studentName || "김싸피", classId: studentData?.classId || 1 };

  const handleStatusChange = Worklets.createRunOnJS((data: { status: string, debug: string }) => { setAiStatus(data.status as AIStatus); setDebugInfo(data.debug); });
  const handleCalibrationFinish = Worklets.createRunOnJS((earTh: number) => { setShowCalibration(false); console.log(`✅ Calibration Finished. EAR Th: ${earTh.toFixed(3)}`); });
  const handleModeChange = (newMode: ClassMode) => { setTimeout(() => { if (newMode === "NORMAL") router.replace("/screens/Classtime_Normal"); else if (newMode === "DIGITAL") router.replace("/screens/Classtime_Digital"); }, 300); };
  ...
  const frameProcessor = useFrameProcessor((frame) => {
    ...
    if (outputs.length > 1) { const scores = outputs[1] as Float32Array; if (scores[0] > MIN_FACE_SCORE) isFaceDetected = true; } 
    else if (outputs.length > 0 && (outputs[0] as Float32Array).length > 100) { isFaceDetected = true; }

    if (!isFaceDetected) { faceMissingCount.value += 1; if (faceMissingCount.value > AWAY_FRAME_LIMIT) { handleStatusChange({ status: "AWAY", debug: "No Face" }); } return; }
    ...
    if (smoothedNosePos.value !== null) { curNoseX = SMOOTHING_ALPHA * rawNoseX + (1 - SMOOTHING_ALPHA) * smoothedNosePos.value[0]; curNoseY = SMOOTHING_ALPHA * rawNoseY + (1 - SMOOTHING_ALPHA) * smoothedNosePos.value[1]; }

    if (!isCalibrated.value) { const newData = [...calibrationData.value, curHead[0], curHead[1], curHead[2], avgEar]; calibrationData.value = newData; smoothedNosePos.value = [curNoseX, curNoseY]; 
      if (newData.length >= CALIBRATION_FRAMES * 4) { let sums = [0, 0, 0, 0]; const count = newData.length / 4; for (let i = 0; i < newData.length; i += 4) { sums[0] += newData[i]; sums[1] += newData[i+1]; sums[2] += newData[i+2]; sums[3] += newData[i+3]; } 
      const avgUserEar = sums[3] / count; let calculatedTh = avgUserEar * 0.6; if (calculatedTh < 0.1) calculatedTh = 0.1; personalEarThreshold.value = calculatedTh; isCalibrated.value = true; handleCalibrationFinish(personalEarThreshold.value); } else { return; } }

    const [yaw, pitch, roll] = curHead;
    let eyesTooLongClosed = false; let eyeDuration = 0;
    if (!eyesOpen) { if (eyeClosedStart.value === null) eyeClosedStart.value = now; lastBlinkTime.value = now; eyeDuration = now - eyeClosedStart.value; if (eyeDuration > EYE_CLOSED_TIME_LIMIT) eyesTooLongClosed = true; } else { eyeClosedStart.value = null; }

    const angleDiff = Math.abs(yaw - lastHeadPose.value[0]) + Math.abs(pitch - lastHeadPose.value[1]);
    lastHeadPose.value = [yaw, pitch, roll];
    let posDiff = 0; let rawDiff = 0; 
    if (smoothedNosePos.value !== null) { const diffX = Math.abs(curNoseX - smoothedNosePos.value[0]); const diffY = Math.abs(curNoseY - smoothedNosePos.value[1]); rawDiff = diffX + diffY; if (rawDiff < MOVEMENT_DEADZONE) posDiff = 0; else posDiff = rawDiff * POS_DIFF_SCALE; }
    smoothedNosePos.value = [curNoseX, curNoseY];
    const totalMovement = (!eyesOpen) ? 0 : (angleDiff + posDiff);
    if (totalMovement > MOVEMENT_THRESHOLD) movementCounter.value += 1; else movementCounter.value = Math.max(0, movementCounter.value - 1);
    const isMoving = movementCounter.value > MOVEMENT_TRIGGER_COUNT;

    let finalStatus: AIStatus = "FOCUS"; let debugStr = `Diff: ${rawDiff.toFixed(2)}`;
    if (eyesTooLongClosed) { finalStatus = "UNFOCUS"; debugStr = `SLEEP (${(eyeDuration/1000).toFixed(1)}s)`; } else if (isMoving) { finalStatus = "UNFOCUS"; debugStr = `MOVING Score: ${totalMovement.toFixed(2)}`; } else { finalStatus = "FOCUS"; }
    handleStatusChange({ status: finalStatus, debug: debugStr });
  }, [model]);

  useEffect(() => { if (!hasPermission) requestPermission(); }, [hasPermission]);
  useEffect(() => { async function lockOrientation() { await ScreenOrientation.lockAsync(ScreenOrientation.OrientationLock.LANDSCAPE_RIGHT); } lockOrientation(); setTimeout(() => setIsCameraInitialized(true), 500); }, []);
  useEffect(() => { if (isSocketConnected && aiStatus !== "RESTROOM" && aiStatus !== "ACTIVITY" && aiStatus !== prevStatusRef.current) { sendStatusToServer(aiStatus); if (aiStatus !== "FOCUS") alertRef.current?.triggerAlert(aiStatus); prevStatusRef.current = aiStatus; } }, [aiStatus, isSocketConnected]);
  useEffect(() => { const client = new Client({ webSocketFactory: () => new SockJS(SOCKET_CONFIG.BROKER_URL), reconnectDelay: SOCKET_CONFIG.RECONNECT_DELAY, onConnect: () => { setIsSocketConnected(true); setStudentCount(1); client.subscribe(SOCKET_CONFIG.SUBSCRIBE.STUDENT_COUNT(classId), (msg) => { try { const data = JSON.parse(msg.body); setStudentCount(data.count || data.studentCount || 0); } catch(e) {} }); client.subscribe(SOCKET_CONFIG.SUBSCRIBE.CLASS_TOPIC(classId), (msg) => {}); client.subscribe(SOCKET_CONFIG.SUBSCRIBE.MODE_STATUS(classId), (msg) => { try { const data = JSON.parse(msg.body); if (data.mode === "NORMAL" || data.mode === "DIGITAL") handleModeChange(data.mode); } catch(e) {} }); setTimeout(() => { client.publish({ destination: SOCKET_CONFIG.PUBLISH.ENTER, body: JSON.stringify({ classid: parseInt(classId), studentld: studentInfo.id, studentName: studentInfo.name }) }); client.publish({ destination: SOCKET_CONFIG.PUBLISH.REQUEST_COUNT, body: JSON.stringify({ classid: parseInt(classId) }) }); }, 100); } }); client.activate(); stompClient.current = client; return () => { if(stompClient.current) stompClient.current.deactivate(); }; }, [classId]);
  const sendStatusToServer = (status: AIStatus) => { if (!isSocketConnected || !stompClient.current) return; let serverType = status === "FOCUS" ? "FOCUS" : status === "AWAY" ? "AWAY" : "UNFOCUS"; const payload = { classid: parseInt(classId), studentld: studentInfo.id, studentName: studentInfo.name, type: serverType, detectedAt: new Date().toISOString() }; stompClient.current.publish({ destination: SOCKET_CONFIG.PUBLISH.ALERT, body: JSON.stringify(payload) }); };
  const handleStudentStatusReport = (status: string) => { const displayStatus = (status === 'RESTROOM' || status === 'ACTIVITY') ? 'FOCUS' : status; setAiStatus(displayStatus as AIStatus); prevStatusRef.current = displayStatus as AIStatus; if (isSocketConnected && stompClient.current) { const payload = { classid: parseInt(classId), studentld: studentInfo.id, studentName: studentInfo.name, type: status, detectedAt: new Date().toISOString() }; stompClient.current.publish({ destination: SOCKET_CONFIG.PUBLISH.ALERT, body: JSON.stringify(payload) }); } };
  ...
  return (
    <View style={styles.container}>
      ...
      <View style={styles.bottomOverlay}><ClassProgressBar targetMinutes={3} /></View>
